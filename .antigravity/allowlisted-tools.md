...
| `Bash` (surgical staging) | conditional (conductor, per-session unlock) | git status/log/diff (read), npm run build/test (scoped), local dev commands on **non-Windows** or WSL-native targets only | **On Frank's Windows host: never Bash for `pwsh`, `.ps1`, or `agy-tools.ps1` — use `command(powershell)`** (see `.antigravity/instructions.md` § Windows Shell Routing). Destructive ops require explicit unlock + audit. Never default. |
