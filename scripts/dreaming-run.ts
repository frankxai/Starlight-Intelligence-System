...
>
> **Format**: \`- <ISO-timestamp> · insights: N · contradictions: N · promotions: N · queued: N · processed: N · decayed: N · archived: N\` (or \`error: <msg>\` on failure). \`queued\` counts NEW candidates appended to memory/PROMOTION_QUEUE.md (after dedup vs memory/.dreaming-state.json).
>
...
function main(): void {
  ...
  (async () => {
    try {
      ...
      // Step 2: Promotions queue writeback
      const state = loadState();
      const queued = appendNewPromotions(dreamResult, state, ts);
