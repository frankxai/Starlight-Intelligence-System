...
describe("LoopEngine", () => {
  ...
  it("allows a node to consume the final available turn and cost unit", () => {
    const chain: LoopGraph = {
      ...
      executorRole: "maker",
      supervisorRole: "checker",
      brakes: { maxTurns: 2, maxCostUnits: 3, emptyRoundsToStop: 1, allowedActions: [], requireWriteback: true },
      nodes: [
  ...
  it("advances through a router branch selected by facts", () => {
    const router: LoopGraph = {
      ...
      executorRole: "maker",
      supervisorRole: "checker",
      brakes: { maxTurns: 4, maxCostUnits: 20, emptyRoundsToStop: 1, allowedActions: [], requireWriteback: true },
      nodes: [
