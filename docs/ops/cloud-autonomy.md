...
built `site/` or `console/`, and `npm publish` was a manual step (the registry
went two majors stale once because of it). This page is the index of every
workflow that closes one of those gaps, plus the exact commands to arm the ones
that are dormant until a secret is set.

**Deploys are not in this list.** Production (`starlightintelligence.org`) and PR
previews are handled by **Vercel's native Git integration** — no GitHub Actions
workflow, no repo secrets. The former `vercel-deploy.yml` CLI pipeline was retired
2026-07-03 (see `DEPLOY.md`). Broken `site/` builds are still caught in CI by the
`harness-check.yml` `web` job on every PR and push.
