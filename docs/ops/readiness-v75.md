...
| 1.5 Mem0 + Graphiti install + `~/captures/` + `Arcanea/wiki/` | `docs/install/mem0.md` + `docs/install/graphiti.md` | Mem0 and Graphiti playbooks exist; install not verified on this machine. Markdown-vault → Graphiti adapter still needs executable validation before Phase 1.7 depends on it. | `staged` | Playbook gap closed; runtime ingestion remains the gate. The next proof is a small import of representative `Arcanea/wiki/` atoms into the temporal graph with receipt + rollback notes. | Frank executes install; Claude Code validates adapter |
| 1.6 Syncthing on Lenovo + Acer + phone | `docs/install/syncthing.md` + `MASSIVE_ACTION_PLAN.md` § 7 | Playbook exists; no verified folder pairs configured in this audit. Phone setup remains GUI-owned. | `staged` | Playbook gap closed; operational gate is verified folder-pair sync for `Starlight-Intelligence-System/`, `Arcanea/wiki/`, `Arcanea/.arcanea/`, `FrankX/content/`, `FrankX/data/`, `Business/`, and `~/captures/`. | Frank executes; Claude Code verifies receipts |
...
### 1. Graphiti runtime validation + Markdown-vault adapter — `staged`
The temporal memory graph is the load-bearing primitive for daily brief (Phase 1.7), Constellation view (Phase 4.1), Meeting Replay (Phase 4.2), and the five voice command primitives (Phase 2.5 — "what changed", "brief me"). `docs/install/graphiti.md` now exists, so the playbook gap is closed. The remaining gate is executable proof: import representative `Arcanea/wiki/` atoms, preserve source metadata, and verify the edge-weight formula `recency × relevance × emotional_salience × strategic_importance` against a small receipt-backed run.

### 2. Syncthing install execution + folder-pair receipt — `staged`
`docs/install/syncthing.md` now carries the setup recipe. The remaining gate is operational verification for the seven named folders (`Starlight-Intelligence-System/`, `Arcanea/wiki/`, `Arcanea/.arcanea/`, `FrankX/content/`, `FrankX/data/`, `Business/`, `~/captures/`). The Acer hot-mirror dependency (risk register § "Single point of failure on Lenovo") gates on Syncthing being live. Phone capture (Phase 1.6) gates on it too.
...
|---|---|---|---|---|
| 1 | Graphiti runtime validation + adapter receipt | Phase 1.5 | Frank executes install; Claude Code validates adapter | Playbook exists. The next ship is executable proof against representative Markdown vault atoms with receipt + rollback notes. |
| 2 | Syncthing folder-pair verification | Phase 1.6 | Frank executes; Claude Code verifies receipts | Playbook exists. The next ship is a verified sync receipt for the seven named folders, including phone/GUI caveats. |
...
| `staged` | 9 (orchestrator promotion path, claude harness alone, screenpipe playbook, meetscribe playbook, mem0 playbook, Graphiti playbook pending runtime validation, Syncthing playbook pending folder-pair receipt, console graph viz, orchestrator-as-IS) |
...
| `unsurfaced` | 11 (daily brief, workspace switching, vercel room, voice primitives wiring, all of Phase 3 spine surfaces, all of Phase 4 except 4.1, vertical wrappers for 5 IS layers, sound-intelligence sub-systems pending parallel agent) |

**Top 5 critical gaps blocking Phase 1:**
1. Graphiti runtime validation + Markdown-vault adapter receipt — playbook exists; runtime proof is still missing.
2. Syncthing folder-pair verification — playbook exists; seven-folder sync receipt is still missing.
