...
import type { MemoryProvider, SISMemoryRecord, TenantMemoryPolicy, ProviderRoute } from './types.js';
...
export function createStarlightMemoryCompatibilityAdapter(options: CompatibilityOptions = {}) {
  ...
  if (usePackage && StarlightMemory) {
    ...
    return {
      routeMemoryRecord: StarlightMemory.routeMemoryRecord || ((record: SISMemoryRecord, policy: TenantMemoryPolicy) => [{ provider: 'local_core' as const, mode: 'canonical_write' as const, reason: 'package delegation fallback' }]),
  ...
  return {
    routeMemoryRecord: (record: SISMemoryRecord, policy: TenantMemoryPolicy = basePolicy): ProviderRoute[] => {
