...
import type { ACOSTrajectory, ACOSPattern } from "./sync.js";
...
function scoreMemoryDepth(memory: MemoryManager): ScoreComponent {
...
function scorePatternQuality(acosPath: string): ScoreComponent {
  let score = 0;
  const details: string[] = [];
  ...
  try {
    ...
    if (patterns.length > 0) {
      const avgSuccess =
        patterns.reduce((sum, p) => sum + p.avgSuccess, 0) / patterns.length;
      const successScore = avgSuccess * 10;
      score += successScore;
      details.push(
        `${(avgSuccess * 100).toFixed(0)}% avg success (${successScore.toFixed(1)}/10)`
      );
    }
    ...
    score += eliteScore;
    details.push(
      `${elite.length} elite patterns (${eliteScore.toFixed(1)}/7)`
    );
  } catch {
...
function scoreOperationalHistory(acosPath: string): ScoreComponent {
  let score = 0;
  const details: string[] = [];
  ...
  if (validCount > 0) {
    ...
    score += avgScore;
    details.push(
      `${(avg * 100).toFixed(0)}% avg success (${avgScore.toFixed(1)}/7)`
    );
  }
...
function scoreLearningVelocity(
  ...
  score += recentScore;
  details.push(
    `${recentMemories.length} memories this week (${recentScore.toFixed(1)}/10)`
  );
  ...
  if (existsSync(acosPath)) {
    ...
    for (const file of files) {
      try {
        ...
        if (traj.startedAt > weekAgo) recentTrajs++;
      } catch {
        // skip
      }
    }
    ...
    details.push(
      `${recentTrajs} trajectories this week (${trajRecencyScore.toFixed(1)}/8)`
    );
  }
...
export function generateIntelligenceReport(
  ...
  const patternsFile = join(acosPath, "patterns.json");
  let acosPatterns: ACOSPattern[] = [];
  try {
    if (existsSync(patternsFile)) {
      acosPatterns = JSON.parse(readFileSync(patternsFile, "utf-8")) as ACOSPattern[];
    }
  } catch { /* skip */ }

  const trajFiles = existsSync(acosPath)
    ? readdirSync(acosPath).filter(
        (f) => f.endsWith(".json") && f.includes("_traj_")
      )
    : [];
