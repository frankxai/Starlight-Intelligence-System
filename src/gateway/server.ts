 ...
 *
 * PRIVACY IS ENFORCED HERE: all read results pass through filterForSearch()
 * from vault-loop.ts. External harnesses carry auth.includePrivate = false
 * and can never override this.
 *
...
import { filterForSearch } from '../vault-loop.js';
import type { VaultLoopEntry } from '../types.js';
...
export class SisGatewayCore {
  ...
  private handleMemorySearch(body: MemorySearchBody | undefined): GatewayResponse {
    ...
    //
    // If in future VaultMemory entries carry privacy flags, add the filter here.
    // For now the contract is: auth.includePrivate is false and enforced structurally.

    return ok(200, { ok: true, results: rawResults });
  }
