...
function nodeEvent(kind: string, cfg: LoopEngineConfig, nodeId: string, actorId: string, evidence: string, now: string): string {
  return JSON.stringify({
    ...
    visibility: "internal",
    retention: "audit",
    summary: kind === "work.blocked" ? `Loop blocked: ${nodeId}` : `Loop node ${nodeId} produced`,
...
function completeEvent(cfg: LoopEngineConfig, now: string, actorId: string, evidence: string): string {
  return JSON.stringify({
    ...
    visibility: "internal",
    retention: "audit",
    summary: `Loop ${cfg.graph.id} completed; checker pass recorded`,
  });
}
...
/** Batch recorder only. The host admits capacity/effort with evaluateLoopGraph,
 * authenticates actors/evidence, and drives convergence rounds. Output mixes
 * __state snapshots and events; filter snapshots before parseWorkGraphJsonl. */
export function runLoopEngine(engine: BuiltLoopEngine, inputs: LoopStepInput[]): string[] {
  ...
  if (!Array.isArray(inputs)) return stop("no-plan");
  ...
  for (const input of inputs) {
    ...
    if (input.proposedAction && (!config.graph.brakes.allowedActions.includes(input.proposedAction) ||
      config.graph.brakes.silenceTriggers?.includes(input.proposedAction))) {
      return stop("silence");
    }
    ...
    if (state.pending.length === 0 && isVerifier) {
      ...
      out.push(completeEvent(config, tick(), input.actor, input.writeback!));
      out.push(JSON.stringify(state));
      return out;
    }
  ...
  if (out.at(-1) !== JSON.stringify(state)) out.push(JSON.stringify(state));
  return out;
}
