...
npm install
npm test       # 256+ tests should pass (auto-builds dist/ for the v01-evals suite)
```

`npm test` chains substrate + operational + v01-evals suites. The v01-evals
suite imports from `dist/` (gitignored, regenerated by `npm run build` which
is invoked automatically inside `test:v01-evals`). If you only want to run
the lighter substrate suite without a build, run `npm run test:substrate`.
