...
**Honest proof available today.** The strongest proof in the entire portfolio, and it is a compliance fact rather than a performance fact: **SIS is the only repo whose documented counts match its filesystem exactly** — 84 skills, 144 agents, 25 commands, verified by file enumeration in `audits/2026-07-25-registry-sweep.md`. In the same sweep, ACOS's own CLAUDE.md undercounts itself by more than 2x (claims 75+/67/35+, actually holds 179/147/173), and Arcanea runs four contradictory values for one library-collection count in production. Against a buyer who has been sold agent platforms on invented dashboards, "we audit our own registry and here is the audit that caught our other repos" is the differentiator. The ownership terms are already written and explicit in §7: client owns the tuned estate, Starlight owns the generalized process, encoded-self boundaries non-negotiable.
