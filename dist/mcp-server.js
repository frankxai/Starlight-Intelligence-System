...
const VAULT_TYPES = ['strategic', 'technical', 'creative', 'operational', 'wisdom', 'horizon'];
function textOf(e) { return e.content ?? e.insight ?? e.wish ?? ''; }
...
function rewriteVault(vaultDir, name, entries) {
    writeFileSync(join(vaultDir, `${name}.jsonl`), entries.map(e => JSON.stringify(e)).join('\n') + '\n', 'utf-8');
}
function findEntry(vaultDir, id) {
    for (const { file, entries } of readVaultFiles(vaultDir)) {
        const entry = entries.find(e => e.id === id);
        if (entry)
            return { entry, vaultName: basename(file, '.jsonl'), all: entries };
    }
    return null;
}
...
export class StarlightMcpServer {
    ...
    registerTools() {
        ...
        this.reg({
            ...
            if (v)
                entries = entries.filter(e => e._vault === v);
        ...
        this.reg({
            ...
            for (const { file, entries } of readVaultFiles(this.vaultDir)) {
                counts[basename(file, '.jsonl')] = entries.length;
        ...
        this.reg({
            ...
            let entries = allEntries(this.vaultDir);
            if (vf)
                entries = entries.filter(e => vf.has(e._vault));
            if (!p.includeExpired)
            ...
            return entries.map(e => {
                ...
                matchedTerms: terms.filter(w => textOf(r).toLowerCase().includes(w)),
            }));
        });
        ...
        this.reg({
            ...
            rewriteVault(this.vaultDir, found.vaultName, found.all.map(e => e.id !== id ? e : { ...e, temporal: { ...defaultTemporal(e), lastConfirmed: now } }));
        ...
        this.reg({
            ...
            rewriteVault(this.vaultDir, found.vaultName, found.all.map(e => {
                if (e.id !== id)
                    return e;
                const out = { ...e, temporal: { ...defaultTemporal(e), validUntil: now } };
                if (reason)
                    out._invalidationReason = reason;
                return out;
            }));
        ...
        this.reg({
            ...
            const stale = allEntries(this.vaultDir).filter(e => isStale(e, th))
                ...
                return { ...r, vault: _vault, daysSinceConfirmed: c ? Math.round((Date.now() - new Date(c).getTime()) / MS_PER_DAY) : -1, isExpired: isExpired(r) };
            });
