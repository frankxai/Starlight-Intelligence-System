import { SIS_ENTRY_TYPES, SIS_VAULT_NAMES, appendCanonicalSisEntry, parseJsonl, readCanonicalSisVault, resolveCanonicalSisHome, } from "./canonical-sis.js";
...
function latestSessionResource(sisRoot) {
...
function patternResource(sisRoot) {
    return {
...
function vaultResource(vault, sisRoot) {
    return {
...
export function getCanonicalSisMcpTools() {
    return [
        {
...
export function getCanonicalSisMcpResources(sisRoot = resolveCanonicalSisHome()) {
    return [
...
export function readCanonicalSisMcpResource(uri, sisRoot = resolveCanonicalSisHome()) {
    if (uri.startsWith("starlight://vaults/")) {
        const vault = uri.split("/").pop();
...
export async function callCanonicalSisMcpTool(name, args = {}, sisRoot = resolveCanonicalSisHome()) {
    if (name === "sis_vault_search") {
        const query = String(args.query || "").trim();
        const limit = Number(args.limit || 10);
        const chosenVault = String(args.vault || "all");
        const targets = chosenVault === "all" ? SIS_VAULT_NAMES : [chosenVault];
    ...
    if (name === "sis_append_entry") {
        const result = appendCanonicalSisEntry(args, sisRoot);
...
//# sourceMappingURL=canonical-sis-mcp.js.map