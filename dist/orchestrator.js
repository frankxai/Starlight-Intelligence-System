 ...
 */
const DEFAULT_EXECUTOR = async (agent, input, _context) => {
...
export class OrchestrationEngine {
    ...
    async execute(task, executor) {
        ...
        const synthesisStrategy = task.synthesis ?? this.selectSynthesisStrategy(pattern);
        ...
        switch (pattern) {
            case "direct":
                executions = await this.direct(task, selectedAgents[0], activeExecutor, executionContext);
                break;
            case "sequential":
                executions = await this.sequential(task, selectedAgents, activeExecutor, executionContext);
                break;
            case "parallel":
                executions = await this.parallel(task, selectedAgents, activeExecutor, executionContext);
                break;
            case "iterative":
                executions = await this.iterative(task, selectedAgents, activeExecutor, executionContext, task.maxIterations ?? 3);
                break;
            case "cascade":
                executions = await this.cascade(task, selectedAgents, activeExecutor, executionContext);
                break;
            case "broadcast":
                executions = await this.broadcast(task, selectedAgents, activeExecutor, executionContext);
                break;
            default:
                executions = await this.direct(task, selectedAgents[0], activeExecutor, executionContext);
        }
     ...
     */
    async direct(task, recommendation, executor, context) {
        if (!recommendation) {
            return [this.createEmptyExecution("no-agent-matched", task.intent)];
        }
        const execution = await this.executeAgent(recommendation.agent, task.intent, executor, context);
        return [execution];
    }
    /**
     ...
     */
    async sequential(task, recommendations, executor, context) {
        ...
        for (const rec of recommendations) {
            ...
            const execution = await this.executeAgent(rec.agent, currentInput, executor, chainContext);
     ...
     */
    async parallel(task, recommendations, executor, context) {
        if (recommendations.length === 0) {
            return [this.createEmptyExecution("no-agents-matched", task.intent)];
        }
        ...
        return results.map((result, index) => {
            ...
            return this.createErrorExecution(recommendations[index].agent.id, task.intent, result.reason instanceof Error ? result.reason.message : String(result.reason));
        });
    }
    /**
     ...
     */
    async iterative(task, recommendations, executor, context, maxIterations) {
        ...
        for (let iteration = 0; iteration < maxIterations; iteration++) {
            ...
            const createExecution = await this.executeAgent(creator.agent, createInput, executor, createContext);
            ...
            if (iteration < maxIterations - 1) {
                ...
                const reviewExecution = await this.executeAgent(reviewer.agent, reviewInput, executor, reviewContext);
     ...
     */
    async cascade(task, recommendations, executor, context) {
        if (recommendations.length === 0) {
            return [this.createEmptyExecution("no-agents-matched", task.intent)];
        }
        const executions = [];
        ...
        const sorted = [...recommendations].sort((a, b) => {
            if (a.agent.type === "specialist" && b.agent.type === "meta")
                return -1;
            if (a.agent.type === "meta" && b.agent.type === "specialist")
                return 1;
            return 0;
        });
        for (let tier = 0; tier < sorted.length; tier++) {
            ...
            const execution = await this.executeAgent(sorted[tier].agent, task.intent, executor, cascadeContext);
            executions.push(execution);
     ...
     */
    async broadcast(task, recommendations, executor, context) {
        if (recommendations.length === 0) {
            return [this.createEmptyExecution("no-agents-matched", task.intent)];
        }
     ...
     */
    synthesize(executions, strategy) {
    ...
    weightedConsensus(executions) {
        ...
        for (const exec of supporting) {
            parts.push(`[${exec.agent}] (confidence: ${exec.confidence.toFixed(2)}): ${exec.output}`);
        }
        return parts.join("\n");
    }
    /**
     ...
     */
    sequentialRefinement(executions) {
    ...
    conflictResolution(executions) {
        ...
        if (highConfidence.length > 0 && lowConfidence.length > 0) {
            parts.push("", "## Resolution Note", `${highConfidence.length} agent(s) showed high confidence while ${lowConfidence.length} showed lower confidence. ` +
     ...
     */
    selectSynthesisStrategy(pattern) {
        switch (pattern) {
            case "direct":
     ...
     */
    defaultAgentCount(pattern, complexity) {
        switch (pattern) {
            case "direct":
                return 1;
            case "sequential":
     ...
     */
    async executeAgent(agent, input, executor, context) {
     ...
     */
    estimateConfidence(output, duration) {
     ...
     */
    writeToMemory(task, pattern, executions, synthesis, confidence) {
        // Only write to memory if the result was confident enough to be useful
        if (confidence < 0.3 || executions.length === 0)
            return false;
     ...
     */
    createEmptyExecution(agentId, input) {
        return {
            agent: agentId,
            input,
     ...
     */
    createErrorExecution(agentId, input, error) {
        return {
            agent: agentId,
            input,
            output: `[Error] ${error}`,
