...
// ── Guidance Generator ───────────────────────────────────────
export function generateGuidance(memory, options) {
...
// ── Data Loaders ─────────────────────────────────────────────
function loadPatterns(acosPath) {
...
function loadTrajectories(acosPath) {
 ...
 */
function distillBehavioralRules(trajectories, patterns) {
    const rules = [];
    // Rule source 1: High-success trajectories that end with verification
    const successfulWithVerify = trajectories.filter((t) => t.successScore >= 0.90 && t.toolBreakdown["Bash"] && t.toolBreakdown["Bash"] >= 2);
    const successfulWithoutVerify = trajectories.filter((t) => t.successScore >= 0.90 && (!t.toolBreakdown["Bash"] || t.toolBreakdown["Bash"] < 2));
    ...
    const readBeforeEdit = patterns.filter((p) => p.sequence.startsWith("Read > Edit") && p.avgSuccess >= 0.80 && p.count >= 2);
    const editWithoutRead = patterns.filter((p) => p.sequence.startsWith("Edit") && !p.sequence.startsWith("Edit > Read > Edit") && p.avgSuccess < 0.70);
    ...
    const withDelegation = trajectories.filter((t) => t.toolBreakdown["Task"] && t.toolBreakdown["Task"] >= 1 && t.operationCount >= 20);
    const withoutDelegation = trajectories.filter((t) => !t.toolBreakdown["Task"] && t.operationCount >= 20);
    ...
    const domainToolSuccess = new Map();
 ...
 */
function distillFailureLessons(trajectories) {
    ...
    for (const [domain, fails] of domainFailures) {
        ...
        if (avgOps > 40) {
            ...
            const successes = trajectories.filter((t) => t.type === domain && t.successScore >= 0.80);
 ...
 */
function generateDomainChecklists(trajectories, patterns) {
    ...
    for (const [domain, trajs] of domainGroups) {
        ...
        const successReadFirst = successful.filter((t) => t.toolBreakdown["Read"] && t.toolBreakdown["Edit"] &&
            t.toolBreakdown["Read"] >= t.toolBreakdown["Edit"]).length;
        ...
        const successVerify = successful.filter((t) => t.toolBreakdown["Bash"] && t.toolBreakdown["Bash"] >= 1).length;
        ...
        if (domain === "deployment") {
            const touchesConfig = successful.filter((t) => t.filesModified.some((f) => f.includes("config") || f.includes(".env") || f.includes("package.json")));
            if (touchesConfig.length > 0) {
                items.push("Verify config/env changes don't break other environments");
            }
        }
        if (domain === "code_development") {
            const touchesComponents = successful.filter((t) => t.filesModified.some((f) => f.includes("component") || f.endsWith(".tsx")));
        ...
        if (successful.length >= 3) {
            const usesTasks = successful.filter((t) => t.toolBreakdown["Task"] && t.toolBreakdown["Task"] >= 1).length;
 ...
 */
function extractCrossProjectInsights(memories, currentProject) {
    const insights = [];
    // Find patterns that appear across multiple projects
    const patternsByContent = new Map();
    for (const mem of memories) {
        if (mem.category === "pattern" && mem.source) {
            const projectMatch = mem.source.match(/project:(\w+)/);
            if (!projectMatch)
                continue; // Skip memories without project tagging
    ...
    for (const [content, projects] of patternsByContent) {
        if (projects.size > 1) {
            ...
            insights.push(`Pattern validated across ${Array.from(projects).join(" + ")}: \`${seq}\``);
        }
    }
    // Find high-confidence lessons from other projects that apply here
    const otherProjectLessons = memories.filter((m) => (m.category === "error" || m.category === "pattern") &&
        m.confidence >= 0.80 &&
