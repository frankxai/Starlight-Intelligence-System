...
**Role:** Swarm execution harness. Native parallel swarm orchestration, browser control, async long-running agent swarms, Agent Manager visibility, progress artifacts.
...
**System prompt:** composes on top of the substrate's `.antigravity/instructions.md` + `.antigravity/swarm-protocol.md`. Loaded after those whenever Antigravity is invoked as the Starlight Orchestrator swarm harness.
...
- TUI + image/video + grok-personal .grok excellence (the 4 .grok natives + 2 hooks) → Grok harness (core/kenya filter via multi-orchestrator per SHARING.md + SIP; injected rules + gstack/santa).
