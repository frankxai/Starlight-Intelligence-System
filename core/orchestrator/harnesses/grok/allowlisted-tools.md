...
**Harness role:** excellence-subagent-mcp. Subagent orchestration (5-12 parallel), MCP symmetry, god 99 e2e QA via repo-mastery + gstack conceptual.
...
| `Task` (Agent / subagent dispatch) | yes (parallel excellence subagents, 5-12 in flight) | Decompose god 99 e2e into focused excellence subagents per gstack pattern | Each subagent prompt must embed "repo-mastery + gstack + Built on SIP + grok-harness attribution". Total scope >200 LOC across subagents → board pre-pass |
...
| `EnterWorktree` / `ExitWorktree` | yes | Isolate parallel subagent worktrees for god-tier e2e without polluting main | Always exit cleanly; attest worktree usage |
...
| `mcp__repo-mastery__*` | god 99 QA | Deep pattern search + evidence collection | Ambiguous match → manual review |
...
- Any claim drift in CLAUDE.md / AGENTS.md / GROK.md / .cursor/* / .gemini/* etc. → block ship, open intel/ remediation, reconcile before next god 99 pass.
...
**gstack QA <99%:**
- Log exact failure matrix. Require remediation subagent loop. Only surface "god 99 e2e" when matrix hits threshold with evidence.
...
## Read before any god 99 e2e invocation
...
When running as Grok excellence harness: **repo-mastery first, gstack verification second, 99-god e2e or block.**
...
- Policy: executable for god 99 e2e subagent/MCP/excellence work
