...
| `antigravity/` | Swarm execution — 96-mind native parallel (define/invoke, browser, async, Agent Manager) via Google Antigravity | yes (2026-06-02) | yes (2026-06-02) | yes (at `.antigravity/mcp-config.json`, swarm-aware) | yes (at `.antigravity/allowlisted-tools.md` + protocol) | **scaffolded** (platform files + harness 2026-06-02 enhancement) |
| `grok/` | TUI + subagents (explore/plan/general) + MCP (github/fs-starlight/git) + image/video + grok-personal excellence seeds opt-in (repo-mastery + multi-harness-orchestrator + excellence-review + harness-integration per SHARING + SIP §5) | yes (2026-06-02) | yes (2026-06-02) | yes (2026-06-02) | yes (2026-06-02) | **scaffolded** (via src/adapters/grok.ts generateAllAdapterFiles + subagent god 99 build; full parity + grok-personal filter + SIP) |
...
**Visual showcases god-99 phase (2026-06-02):** frankx.ai-vercel-website/app/agents/page.tsx + arcanea-ai-app/apps/web/app/[locale]/page.tsx upgraded with 5-Harness Fleet + Grok personal excellence layer sections (glassmorphic, explicit "Core shared. Grok personal excellence seeds + personal creative tools (.grok/personal only, a bit magical, not for everything)", SIP §5, gstack 99+ hammer mandate). Added pure-CSS 3D perspective orbs/constellations with orbiting nodes (Claude/Codex/Gemini/Agy core, Grok .grok personal). TASTE 7-gates, no slop, Frank DNA. These two repos are now the deliberate SOTA visual proofs ("the best thing in the world"). (Later clarified in SHARING + pages to drop opaque codename in favor of descriptive "grok-personal excellence seeds" / "personal creative tools".)
...
1. **`system-prompt.md`** — composes on top of the substrate-level system prompt (or `.antigravity/instructions.md` + `swarm-96-minds-protocol.md` for Antigravity) with orchestrator-routing-aware framing.
...
All files end with `Built on SIP` footer naming the verticals layer. Antigravity files also reference the 96-minds swarm protocol and god-99 excellence standard.
...
### `antigravity/` — Swarm execution harness (96 minds native) — **scaffolded 2026-06-02 → ready to load**

- **Role:** Native Antigravity swarm execution for the full dynamic 96-mind registry (plus 114+ skills). Uses Antigravity primitives (define_subagent, invoke_subagent, Agent Manager, browser control, async progress artifacts) + 1M Gemini-backed context.
- **System prompt:** `core/orchestrator/harnesses/antigravity/system-prompt.md` composes on top of `.antigravity/instructions.md` (sovereign mandate, 96-mind registry, Frank DNA, excellence) + `.antigravity/swarm-96-minds-protocol.md` (full executable checklist, templates, escalation, failure modes, memory commit, cross-harness handoff). God 99 standard.
...
- **Platform files (2026-06-02):** `.antigravity/instructions.md` (expanded full), `swarm-96-minds-protocol.md` (new, detailed), `mcp-config.json` (new), `allowlisted-tools.md` (new).
...
**Phase 1.2 — Add `harnesses/{claude,codex,gemini,opencode}/` with system prompts + MCP scope per CLI + Antigravity swarm harness (2026-06-02):** **DELIVERED (original + enhancement).** Original 12 files + Antigravity harness (2) + `.antigravity/` platform files (instructions expanded + 3 new) + adapter generators + cross-docs. JSONs valid, all SIP footers + excellence integration present. 96-mind swarm protocol integrated.
...
- `MASSIVE_ACTION_PLAN.md` § 4 specifies harness folders (now including antigravity for 96-mind swarm). Original 4 + Antigravity (README + system-prompt at harness level; mcp/allowlist + full instructions + swarm-96-minds-protocol at `.antigravity/` platform level per Antigravity design) delivered with escalation, SIP attestation, excellence. ✓
...
- Substrate non-negotiable "every artifact carries ambient SIP attestation": all original + new Antigravity files (harness + .antigravity/ + adapter + updated docs) end with `Built on SIP` footer + layers citation. 96-minds + swarm-protocol + god-99 excellence integrated. ✓
...
- Layers used: [file-contract, attestation, sovereignty, archetype-extension, 96-minds-dynamic-registry, swarm-protocol]
- Verticals: core/orchestrator (layer 10 — master Intelligence System), .antigravity (platform adapter + swarm harness)
- Generated: 2026-04-26 (original) + 2026-06-02 (Antigravity 96-mind swarm enhancement pass — full instructions, swarm-96-minds-protocol, mcp-config, allowlisted, harness scaffold, adapter generators, docs integration)
- Closes: v7.5 board REVISE Item 4 option (b) + swarm execution surface (Antigravity harness + 96 minds protocol).
- Excellence: God 99. All new content SIP-attested, load-def-first, board-gated, vault-canonical.
...
- ACOS: `adapters/grok/index.ts` (full read) already had 4 Kenya seeds + Kenya magical notes + gstack + SIP + "Phase 5 harness parity 5-way"; `install.sh` (harness sections read) had explicit 4 seeds (harness-integration/excellence-review/repo-mastery/multi-harness-orchestrator) + 2 json hooks + "never leak" + agy mentions in delegation + "Kenya .grok only (opt-in)". No HARNESS-STATUS.md present in ACOS (only in SIS). No harnesses/ dir (cf. SIS `core/orchestrator/harnesses/{antigravity,grok,...}/`); no .antigravity/ seeds (cf. SIS adapter + platform files).
...
- ACOS `adapters/grok/index.ts`: 2 small adds to header + Kenya comment block (explicit "Kenya filter notes", "EXPLICIT 4 SKILLS LIST", "5 FLEET" + agy, gstack/SIP) + footer SIP attest (2026-06-02 ACOS upgrade note + 5-parity).
...
- claude-code-config `SHARING.md`: small append verification block ("2026-06-02 verification + ACOS upgrade note": confirms current SHARING + grok seeds, lists cross-repo coverage, 5-fleet + Kenya).
...
- SIS `core/orchestrator/HARNESS-STATUS.md`: this append section (today's 5-parity + Kenya; evidence + what upgraded; closes "beyond SIS" for ACOS/claude-code-config/frankx visuals). No ACOS HARNESS-STATUS created (per "if present").
...
- Explicit descriptive everywhere: "grok-personal excellence seeds (.grok only)" / "personal-creative" / "a bit magical, not for everything" / "sovereign" / "stay in ~/.grok/skills + ~/.grok/hooks ONLY" / "never leak" / "core/personal filter" / "greps confirm 0 leaks" / "/sip-share-audit" + SIP §5 encoded-self. 4 seeds always listed (repo-mastery, multi-harness-orchestrator, excellence-review, harness-integration) + gstack (core, "visual hammer" 99+).
...
- All SIP-attested; Frank DNA; God 99 gates. Codename removed; descriptive + Why explanation in SHARING.
...
**All works. God 99 lead.** Small reversible. SIP everywhere. 0 leaks preserved/enforced. Ready for `starlight` wrapper + full agy scaffold activation.
...
- 2026-06-02: ACOS/SIS/claude/frankx visual 5-parity + Kenya (agy + grok) upgrade pass — adapter/install/docs/SHARING/status/comments enhanced; scaffold proposed via install; visuals confirmed; HARNESS-STATUS updated. Small reversible. God 99.
