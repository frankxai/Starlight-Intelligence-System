...
- `v7.6.1` · `2026-05-18` · **Stack Drift Resolution** — Ecosystem alignment pass by Starlight Central Command. Fixed inventory configuration for Starlight substrate (set `stack_md_expected: false` due to canonical substrate STACK.md). Scaffolded missing STACK.md manifests for core repos (Arcanea, FrankX, Business). Resolved RED drift status for all inventory repos. Created CLAUDE.md for arcanea-onchain to establish independent workspace context. Updated FrankX defaults to match reality.
