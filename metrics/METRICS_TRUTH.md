...
`/metrics/current.json` is the living ledger. Every entry MUST include `last_verified` (ISO date) and a `source`. If a metric is not in the ledger, do not invent it — either leave it out, use a qualified range, or add it to the ledger first with a verified source.
...
| supported         | Frank advised, reviewed, or amplified the work in a supporting role.    |
...
For enterprise/commercial impact: never claim ownership of a full deal unless the ledger records it as `built` or `contributed_to` with a verified source.
