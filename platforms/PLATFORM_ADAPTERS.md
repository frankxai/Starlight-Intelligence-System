...
- **Antigravity** uses `.antigravity/` (instructions + swarm-protocol + mcp + allowlist) with Gemini-backed 1M context + native agent swarm primitives (define_subagent, invoke_subagent, Agent Manager, browser control, progress artifacts). 2026-06-02 enhancement adds full protocol + orchestrator harness integration.
...
- **Agent Swarm** — Native `define_subagent` + `invoke_subagent` of the full dynamic SIS registry (the agent registry + skills). See `.antigravity/swarm-protocol.md` (checklist, templates, escalation, excellence patterns) and `core/orchestrator/harnesses/antigravity/system-prompt.md`.
...
| **Antigravity** | Agent-first IDE, browser control, async patterns, native agent swarm (define/invoke, Agent Manager) + orchestrator harness | `.antigravity/` (full 2026-06-02: instructions + swarm-protocol + mcp + allowlist) + `core/orchestrator/harnesses/antigravity/` |
