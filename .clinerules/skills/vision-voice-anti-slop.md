# Skill: vision/voice-anti-slop
> Enforces the anti-slop voice contract for every user-facing prose surface — refuses AI-slop signifier words and patterns, preferring architectural vocabulary that names specific files, commands, dates, and deliverables. Use whenever generating site copy, vault entries, plan documents, handover artifacts, PR descriptions, or any artifact attested with "Built on SIP." Loaded automatically by any agent emitting user-facing prose.
