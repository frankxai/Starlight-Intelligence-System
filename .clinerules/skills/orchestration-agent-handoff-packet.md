# Skill: orchestration/agent-handoff-packet
> Build a structured handoff packet so any frontend (voice, text, intake, council) can hand work to any deep agent (Claude Code, Codex, Gemini, vertical agents) without re-explaining context. The contract that lets cockpit and engine room compose without friction.
