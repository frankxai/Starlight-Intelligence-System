...
The single CLI Frank should type into is `starlight`. It wraps and orchestrates the four model CLIs.
...
- Each CLI gets its own agent harness folder under `core/orchestrator/harnesses/{claude,codex,gemini,opencode}/` with: system prompt, MCP config, allowlisted tools, escalation rules.
...
| 1.2 | Add `harnesses/{claude,codex,gemini,opencode}/` with system prompts + MCP scope per CLI | Claude Code | Path exists |
